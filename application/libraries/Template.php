...
class CI_Template {
      ...
      var $base_url       = '/';
      var $assets_js      = 'assets/js/';
      var $assets_css     = 'assets/css/';
      var $autoload_lib   = array();
   ...
   function initialize($props)
   {
      ...
      if (isset($props['autoload_lib'])) {
         $this->autoload_lib = array_merge($this->autoload_lib, $props['autoload_lib']);
      }

      // Load partials
      if (isset($props['partials']))
      {
         $this->set_partials($props['partials']);
      }
   }
   ...
   /**
    * Set partials for writing to
    *
    * @access  public
    * @param   array   properly formed partials array
    * @return  void
    */

   function set_partials($partials)
   {
      if(count($partials) && is_array($partials) && !empty($partials))
      {          
          if (count($this->autoload_lib) > 0) {
            $this->CI->load->library($this->autoload_lib);
          }
          ...
          {
               if (file_exists(APPPATH .'views/partials/'. $partial . EXT)) 
               {
                  if($this->parse_template)
                  {
                     $this->regions[$partial]['content'][] = $this->CI->parser->parse('partials/'.$partial, '', TRUE);
                  }
                  else
                  {
                     $this->regions[$partial]['content'][] = $this->CI->load->view('partials/'.$partial, '', TRUE);
                  }                  
               }
          }
      }
   ...
   /**
    * Dynamically include Javascript asset file in template
    * 
    * NOTE: This function does NOT check for existence of .js file
    *
    * @access  public
   ...
   {
      return $this->add_js($this->assets_js.$file, 'import'); 
   }
   ...
   /**
    * Dynamically include CSS asset file in template
    * 
    * NOTE: This function does NOT check for existence of .js file
    *
    * @access  public
    * @param   string   css to import
    * @return  TRUE on success, FALSE otherwise
    */

   function asset_css($file = '')
   {
      return $this->add_css($this->assets_css.$file, 'link'); 
   }
